...
def query_database():  # Tab 1
    ...
    if parse_warnings:
        status += f" \n Cảnh báo: {len(parse_warnings)} dòng bị bỏ qua."
    progress_label.config(text=status)


def reverse_query_database():  # Tab 2
    ...
    # Dựng truy vấn SELECT trả về toàn bộ cột + batching theo giới hạn tham số
    ...
    try:
        ...
        for i in range(0, len(ids), batch_size):
            ...
            sql = f"""
                SELECT {quoted_cols}
                FROM lot_traceback
                WHERE internal2did_id IN ({placeholders})
            """
...
def query_by_lot_database():  # Tab 3 (MỚI)
...
def clear_all():
    ...
    try:
        text_input_tab3.delete("1.0", tk.END)  # Tab 3 (mới)
    except Exception:
        pass
...
# Hàng 4: Progress bar and status
progress_label = tk.Label(root, text="", anchor="w")
progress_label.grid(row=2, column=0, sticky="w", padx=5)
progress_bar = ttk.Progressbar(root, mode="indeterminate")
progress_bar.grid(row=2, column=1, columnspan=3, sticky="ew", padx=5)
